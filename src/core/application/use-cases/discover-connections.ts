...
export class DiscoverConnectionsUseCase {
  ...
  async execute(sourceNoteId: string): Promise<CrossDomainConnection[]> {
    ...
    for (const [targetNoteId, targetEmbedding] of allEmbeddings) {
      ...
      if (targetPath && this.isExcludedPath(targetPath)) continue;
      ...
      try {
        ...
        // 8. 유사도 임계값 필터링
        if (similarity < this.options.minSimilarity) continue;
        ...
        // 11. 창발성 임계값 필터링
        if (serendipityScore.value < this.options.minSerendipityScore) continue;
        ...
        // 분류 실패한 노트는 스킵
        console.warn(`[CDC] Failed to classify note ${targetNoteId}:`, error);
        continue;
      }
    }
